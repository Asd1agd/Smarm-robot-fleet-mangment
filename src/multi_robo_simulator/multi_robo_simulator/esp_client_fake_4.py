...
class PathExecutor(Node):
    def __init__(self):
        ...
        self.cmd_pub = self.create_publisher(Twist, '/robot4/cmd_vel', 10)
        self.status_pub = self.create_publisher(String, '/bot4/status', 10)

        self.create_subscription(PathArray, '/bot4/path', self.path_callback, 10)
        self.create_subscription(JointState, '/robot4/joint_states', self.joint_callback, 10)
        self.pose_log = []  # List to store global x, y, w
        self.create_subscription(Odometry, '/robot4/Odometry', self.odom_callback, 10)
