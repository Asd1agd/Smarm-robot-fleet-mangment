...
class PathExecutor(Node):
    def __init__(self):
        super().__init__('esp_client_fake_6')

        self.cmd_pub = self.create_publisher(Twist, '/robot6/cmd_vel', 10)
        self.status_pub = self.create_publisher(String, '/bot6/status', 10)

        self.create_subscription(PathArray, '/bot6/path', self.path_callback, 10)
        self.create_subscription(JointState, '/robot6/joint_states', self.joint_callback, 10)
