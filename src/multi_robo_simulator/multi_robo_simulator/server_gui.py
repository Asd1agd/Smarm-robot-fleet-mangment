...
class ServerGUI(Node):
    ...
    def init_bot(self, bot_id):
        ...
        # self.create_subscription(Odometry, f"/{bot_id}/odom", lambda msg, b=bot_id: self.update_odom_from_tf(b), 10)
    ...
    def set_initial_pose(self, bot_id, msg):
        x = msg.position.x + 35.3
        y = msg.position.y + 22.1
        yaw = self.quaternion_to_yaw(msg.orientation)
        yaw_deg = math.degrees(yaw)
        yaw_new = math.radians(yaw_deg - 90)

        self.robots[bot_id]['pos'] = (x * GRID_SIZE, y * GRID_SIZE)
        if self.robots[bot_id]['yaw'] is None:
            self.robots[bot_id]['yaw'] = yaw_new
        self.get_logger().info(f"[SPAWN_POSE] {bot_id} POS: ({x:.2f}, {y:.2f}), YAW: {math.degrees(yaw_new):.2f} deg")
    ...
    # def update_odom_from_tf(self):
    #     for bot_id in self.robots.keys():
    #         robot_name = bot_id.replace("bot","robot")
    #         frame = f"{robot_name}/base_footprint"
    #         try:
    #             trans = self.tf_buffer.lookup_transform('map', frame, rclpy.time.Time())

    #             x = trans.transform.translation.x + 35.3
    #             y = trans.transform.translation.y + 22.1
    ...
    #             row = int(y * GRID_SIZE // GRID_SIZE)
    #             col = int(x * GRID_SIZE // GRID_SIZE)
    #             cell = (row, col)
    ...
    #             self.get_logger().warn(f"Transform not available for {bot_id}: {str(e)}")


    def update_status(self, bot_id, msg):
        bot_status = msg.data
        self.robots[bot_id]['status'] = bot_status
        self.get_logger().debug(f"bot_status: {bot_status}")
        if ("reached" in bot_status):
            c_cmd = self.global_commands[bot_id].pop(0)
            # if "reached" in bot_status:
            #     self.global_commands.pop(0)
            if c_cmd > 0:
                for i in range(int(c_cmd)):
                    self.path_cells[bot_id].pop(0)
            bots = self.path_cells  #{'bot1': [(0, 1), (0, 2), (1, 2), (2, 2)],'bot2': [(3, 3), (3, 4), (4, 4)],'bot3': [(5, 0), (5, 1), (5, 2)],}
            print("bots: ",bots)
            x = bots[bot_id][0][1] + 0.5
            y = bots[bot_id][0][0] + 0.5
            self.robots[bot_id]['pos'] = (x * GRID_SIZE, y * GRID_SIZE)
            if c_cmd == -3:
                self.robots[bot_id]['yaw'] -= 1.57
            elif c_cmd == -6:
                self.robots[bot_id]['yaw'] += 1.57
    ...
    def loop(self):
        # self.update_odom_from_tf()  # Call TF-based odom update once per loop
        self.render_map()
        for bot_id, data in self.robots.items():
            ...
            self.canvas.create_text(x, y - 15, text=bot_id, fill='red', tags=('robot', bot_id))
            self.canvas.create_text(x, y + 15, text=data['status'], fill='red', tags=('robot', bot_id))
            if data['goal']:
                gx, gy = data['goal']
    ...
    def double_click(self, event):
        if self.drawing_path and self.selected_robot:
            ...
            self.global_commands[self.selected_robot] = commands
            msg = PathArray()
            msg.path = commands
