...
class PathExecutor(Node):
    def __init__(self):
        ...
        self.status_pub = self.create_publisher(String, '/bot2/status', 10)

        self.create_subscription(PathArray, '/bot2/path', self.path_callback, 10)
        self.create_subscription(JointState, '/robot2/joint_states', self.joint_callback, 10)
        self.pose_log = []  # List to store global x, y, w
        self.create_subscription(Odometry, '/robot2/Odometry', self.odom_callback, 10)
